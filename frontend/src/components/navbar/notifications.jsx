...
const NotificationLayout = ({ data, notifStatus, handleAction, NOTIFICATION_TYPES }) => {
  ...
  const sendAction = useCallback(
    action => {
      const messageType =
        action === 'accepted' ? NOTIFICATION_TYPES.ACCEPT_FRIEND : NOTIFICATION_TYPES.REJECT_FRIEND;
      sendMessage(
        JSON.stringify({
          type: messageType,
  ...
  return (
    <div className="group relative mt-2 flex w-full max-w-96 border-b border-gray-700/30 p-4 transition-all duration-300 hover:bg-gray-800/30">
      ...
      <div className="ml-4 text-sm font-normal">
        ...
        <div className="mb-3 mt-1 max-w-52 text-sm font-normal text-gray-300">{data.message}</div>
    ...
    <div>
      <div className="notifications">
        <div className="relative border border-gray-400 p-2 rounded-full transition duration-200 cursor-pointer">
          ...
          />
          <span
            className={`absolute top-0 left-10 text-white rounded-full px-2 ${
