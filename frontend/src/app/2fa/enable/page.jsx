...
import { apiTwoFactorAuthQrcode, apiEnableTwoFactorAuth, apiTwoFactorAuthIsEnabled } from '@/services/twoFactorAuthApi';
...
const QrCodePage = () => {
    ...
    if (redirect) {
        return (
            <div className="flex flex-col items-center justify-center w-fit h-fit mx-auto">
                Redirecting... wait...
            </div>
        ); 
    }
    ...
    return (
        <div className="relative flex flex-col items-center justify-center w-screen h-auto ">
            <div className='m-4 flex flex-col items-start gap-3 justify-evenly flex-wrap w-fit h-fit  p-1  border-white/20'>
                <div className='capitalize text-lg text-center'>turn on 2-step verification</div>
                ...
                <Button onClick={handleEnable} className='flex justify-center font-mono items-center bg-blue-400 hover:bg-white hover:text-black px-4 py-2'>
                    Enable
                </Button>
            </div>
        </div>
